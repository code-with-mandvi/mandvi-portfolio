...
function attachTilt(selector, maxTilt = 9) {
  document.querySelectorAll(selector).forEach((el) => {
    el.addEventListener("mousemove", (event) => {
      ...
      el.style.transform = "rotateX(" + rotateX.toFixed(2) + "deg) rotateY(" + rotateY.toFixed(2) + "deg) translateY(-6px)";
    });
...
attachTilt(".project-3d", 7);
attachTilt(".skill-orb", 6);

if (sceneShell) {
  document.addEventListener("mousemove", (event) => {
    const x = (window.innerWidth / 2 - event.clientX) / 42;
    const y = (window.innerHeight / 2 - event.clientY) / 42;
    sceneShell.style.transform = "rotateY(" + x.toFixed(2) + "deg) rotateX(" + (-y).toFixed(2) + "deg)";
  });
}
...
if (particleCanvas) {
  ...
  function drawParticles() {
    ...
    particles.forEach((p) => {
      ...
      const size = (1 + p.z * 2.4) * window.devicePixelRatio;
      ctx.beginPath();
      ctx.arc(p.x * w, p.y * h, size, 0, Math.PI * 2);
      ctx.fillStyle = "rgba(" + (p.z > 0.5 ? "255, 61, 167" : "19, 184, 255") + ", " + (0.18 + p.z * 0.28) + ")";
      ctx.fill();
    });
